...
public:
    ...
    static inline void swap(Type& left, Type& right)
    {
        ...
        __asm__( 
            "cmp %[right_key],%[left_key]\n\t" 
            "csel %[left_key],%[right_key],%[left_key],LO\n\t"
            "csel %[right_key],%[tmp],%[right_key],LO\n\t"
...
public:
    ...
    static inline void swap(Type& left, Type& right)
    {
        ...
        __asm__( 
            "cmp %[right_key],%[left_key]\n\t"
            "csel %[left_key],%[right_key],%[left_key],LO\n\t" 
            "csel %[left_reference],%[right_reference],%[left_reference],LO\n\t"
            "csel %[right_key],%[tmp],%[right_key],LO\n\t"
            "csel %[right_reference],%[tmp_ref],%[right_reference],LO\n\t"
            : [left_key] "=&r"(left.key), [right_key] "=&r"(right.key), [left_reference] "=&r"(left.reference), [right_reference] "=&r"(right.reference)
            : "0"(left.key), "1"(right.key), "2"(left.reference), "3"(right.reference), [tmp] "r"(tmp), [tmp_ref] "r"(tmpRef)
            : "cc" 
        );
...
public:
    ...
    static inline void swap(Type& left, Type& right)
    {
        ...
        __asm__ volatile(
            ...
            "csel %[left_pointer],%[right_pointer],%[left_pointer],LO\n\t"
        ...
        leftPointer = &tmp;
        __asm__ volatile(
            "csel %[right_pointer],%[left_pointer],%[right_pointer],LO\n\t"
